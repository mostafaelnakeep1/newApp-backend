...
exports.deleteProduct = exports.toggleProductVisibility = exports.toggleProductSuspension = exports.getAllProducts = exports.getPublicProducts = exports.updateProduct = exports.getProducts = exports.createProduct = void 0;
...
const createProduct = async (req, res) => {
    try {
        ...
        const { error, value } = productValidation_1.createProductSchema.validate(req.body, { abortEarly: false });
        if (error) {
            return res.status(400).json({ errors: error.details.map((d) => d.message) });
        }
        const { description, price, image, type, capacity, brand, installDuration, companyId: incomingCompanyId, } = value;
...
const getProducts = async (req, res) => {
    try {
        const { page = "1", limit = "10", keyword = "", minPrice, maxPrice, companyId, isSuspended, isHidden, } = req.query;
        ...
        const total = await Product_1.default.countDocuments(filters);
        const products = await Product_1.default.find(filters)
...
const updateProduct = async (req, res) => {
    try {
        ...
        const { description, price, type, capacity, brand, installDuration } = updates;
        if (description)
        ...
        if (brand)
            product.brand = brand;
        if (installDuration)
            product.installDuration = installDuration;
...
const getPublicProducts = async (req, res) => {
    try {
        const products = await Product_1.default.find({ isHidden: false }).populate("company", "name");
        res.json({ products });
    }
    catch (error) {
...
const getAllProducts = async (req, res) => {
    try {
        ...
        const { search, brand, power, minPrice, maxPrice, sortBy, } = req.query;
        const filters = {};
        if (brand) {
            filters.brand = brand;
        }
...
const toggleProductSuspension = async (req, res) => {
    try {
...
exports.toggleProductSuspension = toggleProductSuspension;
